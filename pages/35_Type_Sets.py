...
# Create tabs - Added Set Summary as second tab
tabs = st.tabs(["📊 My Sets", "📋 Set Summary", "➕ Define Set", "✏️ Modify Set"])
...
with tabs[1]:
    ...
    if not type_sets:
        ...
        # Get value and cost data for each set
        summary_data = []

        for type_set in type_sets:
            set_id = type_set['id']

            # Get basic summary from the view
            summary = get_type_set_summary(set_id)

            # Get value and cost data for owned coins in this set
            value_query = """
                SELECT 
        ...
        total_sets = len(summary_data)
        total_coins_needed = sum([s.get('total_coins', 0) if s else 0 for s in
                                  [get_type_set_summary(ts['id']) for ts in type_sets]])
        total_coins_owned = sum([s.get('coins_owned', 0) if s else 0 for s in
                                 [get_type_set_summary(ts['id']) for ts in type_sets]])

        col1, col2, col3 = st.columns(3)
        col1.metric("Total Type Sets", total_sets)
        col2.metric("Total Coins Needed", total_coins_needed)
        col3.metric("Total Coins Owned", total_coins_owned)
        ...
        st.download_button(
            ...
            data=csv,
            file_name="type_set_summary.csv",
            mime="text/csv"
        )

# =====================================================
# Tab 3: Define Set - Create new sets with criteria
# =====================================================
with tabs[2]:
...
# =====================================================
# Tab 4: Modify Set - Add/remove coins from existing sets
# =====================================================
with tabs[3]:
    type_sets = get_all_type_sets()
    ...
    if not type_sets:
        ...
        # Select set to modify - ALSO REMOVED ID HERE
        set_options = {s['name']: s for s in type_sets}
            ...
            if all_types:
                ...
                if available:
                    ...
                    if len(available) > display_limit:
                        st.caption(
                            f"Showing first {display_limit} of {len(available)} available coins")
